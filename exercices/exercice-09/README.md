...
1. Vérifiez que le conteneur OpenLDAP est actif (les données initiales sont injectées automatiquement au démarrage via le fichier `infrastructure/openldap/bootstrap.ldif`) :
   ```bash
   docker ps | grep autheria-openldap
   ```

2. Vérifiez que les données sont bien présentes (recherche ciblée dans `ou=people` avec les attributs utiles) :
   ```bash
   docker exec autheria-openldap ldapsearch -x -b "ou=people,dc=registre,dc=valdoria,dc=local" -D "cn=admin,dc=registre,dc=valdoria,dc=local" -w admin "(objectClass=inetOrgPerson)" dn uid cn mail l
   ```

   Vous devez voir les trois sujets : `elara`, `thorin` et `aldric` avec leurs attributs (uid, cn, mail, l).

   **Si aucune entrée n’apparaît** : le bootstrap n’a peut‑être pas été chargé (volume déjà existant). Depuis le dossier `infrastructure/`, recréez les volumes puis relancez : `docker compose down -v && docker compose up -d`, puis relancez la commande ci‑dessus.
...
| **Recherche utilisateurs : « No Such Object » / NameNotFoundException** (DN `ou=people,dc=registre,dc=valdoria,dc=local`) | L’arbre LDAP n’existe pas (bootstrap non appliqué) | Depuis `infrastructure/` : `docker compose down -v && docker compose up -d`. Vérifiez avec la commande `ldapsearch` de l’étape 1 avant de reconfigurer Keycloak. |
