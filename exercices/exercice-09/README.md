...
Office du Maître des Registres (OpenLDAP)
    ...
    │
    └── ou=groups
          ├── cn=guilde-marchands  → member: thorin
          └── cn=gouverneur        → member: aldric
```
...
| **Group Mapper** | Groupe `guilde-marchands` | Groupe `guilde-marchands` |
| **Role Mapper** | Groupe `gouverneur` | Realm role `gouverneur` |
...
2. Injectez les données initiales dans l'annuaire :
   ```bash
   docker exec -i autheria-openldap ldapadd -x -D "cn=admin,dc=registre,dc=valdoria,dc=local" -w admin < infrastructure/openldap/bootstrap.ldif
   ```

3. Vérifiez que les données sont bien présentes :
   ```bash
   docker exec autheria-openldap ldapsearch -x -b "dc=registre,dc=valdoria,dc=local" -D "cn=admin,dc=registre,dc=valdoria,dc=local" -w admin "(objectClass=inetOrgPerson)" uid mail l
   ```
