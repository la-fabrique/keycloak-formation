...
- Observer un jeton JWT réel via les outils développeur du navigateur
- Comprendre la différence entre rôles de royaume et rôles de client
- Analyser la structure d'un jeton JWT (header, payload, signature)
...
```json
{
  "exp": 1771336689,
  "iat": 1771336389,
  "jti": "onrtna:1ffaaf15-ec1c-4dcc-e14d-badaf547e215",
  "iss": "http://localhost:8080/realms/valdoria",
  "aud": "account",
  "sub": "2937527b-b2ad-4844-b70b-6d43acd7428e",
  "typ": "Bearer",
  "azp": "account-console",
  "sid": "Y5ZqoJWJHUeICm0Q6G_RUzta",
  "acr": "1",
  "resource_access": {
    "account": {
      "roles": [
...
| `aud` | Audience — l'API cible pour laquelle le jeton est destiné |
| `sub` | Subject — identifiant unique de l'utilisateur |
| `azp` | Authorized party — le client qui a demandé le jeton |
| `sid` | Session ID — identifiant de la session utilisateur |
| `scope` | Les scopes OAuth2 demandés (openid, email, profile) |
...
| `name` | Nom complet |
| `resource_access.account.roles` | Rôles spécifiques au client `account` |
...
### Étape 8 — Récapitulatif : comprendre les rôles de royaume vs rôles de client
...
| Client role | Permission sur un service spécifique |
...
| Account Console | Bureau des affaires civiles |
| Outil Evaluate | Atelier de prévisualisation des laissez-passer |
| Audience (aud) | API cible du laissez-passer |
